...
export class TableRenderer extends Component {
  ...
  renderColHeaderRow(attrName, attrIdx, pivotSettings) {
    ...
    const attrNameCell = (
      <th
        ...
        data-column-header={namesMapping[attrName] || attrName}
      >
        {displayHeaderCell(
          needToggle,
          subArrow,
      ...
      if (attrIdx < colKey.length) {
        ...
        attrValueCells.push(
          <th
            ...
            data-column-header={
              namesMapping[headerCellFormattedValue] || headerCellFormattedValue
            }
            colSpan={colSpan}
            rowSpan={rowSpan}
        ...
        {rowAttrs.map((r, i) => {
          ...
          return (
            <th
              className="pvtAxisLabel"
              key={`rowAttr-${i}`}
              data-column-header={namesMapping[r] || r}>
              {displayHeaderCell(
                needLabelToggle,
                subArrow,
                arrowClickHandle,
  ...
  renderTableRow(rowKey, rowIdx, pivotSettings) {
    ...
    const valueCells = visibleColKeys.map(colKey => {
      ...
      const attrIdx = 0;
      const attrCol = colAttrs[attrIdx];

      const headerCellFormattedValue =
        dateFormatters &&
        dateFormatters[attrCol] &&
        typeof dateFormatters[attrCol] === 'function'
          ? dateFormatters[attrCol](colKey[attrIdx])
          : colKey[attrIdx];
      ...
      return (
        <td
          ...
          data-column-header={headerCellFormattedValue}
          data-column-row-header={pivotSettings.visibleRowKeys[flatRowKey] || flatRowKey}
